...
class RNNCell(object):
    ...
    def forward(self, x, h_prev_t):
        ...
        """
        aff = np.dot(x, np.transpose(self.W_ih)) + np.dot(h_prev_t, np.transpose(self.W_hh)) + self.b_ih + self.b_hh
        h_t = self.activation(aff)
        return h_t
    ...
    def backward(self, delta, h_t, h_prev_l, h_prev_t):
        ...
        self.dW_ih += np.matmul(np.transpose(dz), h_prev_l)/batch_size  # TODO
        self.dW_hh += np.matmul(np.transpose(dz), h_prev_t)/batch_size  # TODO
