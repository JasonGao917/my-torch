...
class RNNPhonemeClassifier(object):
    ...
    def forward(self, x, h_0=None):
        ...
        for i in range(seq_len):
            h = np.zeros((self.num_layers, batch_size, self.hidden_size), dtype=float)
            x_c = self.x[:, i, :]
            y_c = self.hiddens[-1][0]
            h[0] = self.rnn[0].forward(x_c, y_c)
            for j in range(1, self.num_layers):
                y_cc = self.hiddens[-1][j]
                x_cc = h[j - 1]
                h[j] = self.rnn[j].forward(x_cc, y_cc)
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for i in range(seq_len, 0, -1):
            for j in range(self.num_layers - 1, 0, -1):
                delta = dh[j]
                h_t = self.hiddens[i][j]
                h_prev_l = self.hiddens[i][j - 1]
                h_prev_t = self.hiddens[i - 1][j]
                dx, dh_prev_t = self.rnn[j].backward(delta, h_t, h_prev_l, h_prev_t)
                dh[j] = dh_prev_t
                dh[j - 1] += dx
            delta0 = dh[0]
            h_t0 = self.hiddens[i][0]
            h_prev_l0 = self.x[:, i - 1, :]
            h_prev_t0 = self.hiddens[i - 1][0]
            dh[0] = self.rnn[0].backward(delta0, h_t0, h_prev_l0, h_prev_t0)[1]
        dh_0 = dh/batch_size
